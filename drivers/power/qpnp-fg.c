...
static int update_sram_data(struct fg_chip *chip, int *resched_ms)
{
	...
	pr_debug("soc:[%d], soc_raw[%d], voltage:[%d], ocv:[%d], current:[%d], "
...
static int fg_batt_print_work(struct fg_chip *chip)
{
	...
	pr_info("[FG_INFO]:RS=0x%X, BS=%d, CS=%d, QS=%d, "
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	if (rc < 0) {
		pr_err("%s:won_test File dev_attr_at_chcomp creation failed: %d\n",
				__func__, rc);
		rc = -ENODEV;
		goto err_rt_current;
	}
