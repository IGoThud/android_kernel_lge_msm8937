...
struct battery_id_type battery_id_list[] = {
	{
		...
		.battery_type_name = "LGE_BL45B1F_Tocad_3000mAh",
	},
	{
		.battery_id = BATT_ID_RA4301_VC1,
		.battery_cell_type = LGC_LLL,
		.battery_type_name = "LGE_BL45B1F_LGC_3000mAh",
	},
	{
	...
	},
	{
		...
		.battery_cell_type = LGC_LLL,
		.battery_type_name = BATT_ID_DEFAULT_TYPE_NAME,
	},
};
	...
	if (smem_batt == NULL) {
		...
		if (_smem_batt_id == BATT_ID_DEFAULT)
			for (i = 0; i < BATT_ID_LIST_MAX;i++) {
				if (BATT_ID_DEFAULT == battery_id_list[i].battery_id) {
					strcpy(battery_id_list[i].battery_type_name, info->default_battery);
