...
//LGE_CHANGE_S, Workaround Sof Freeze patch
void msm_isp_handle_pingpong_mismatch(
	struct vfe_device *vfe_dev)
{
	struct msm_isp_event_data error_event;
	error_event.frame_id =
	...
	msm_isp_send_event(vfe_dev, ISP_EVENT_ERROR, &error_event);
	pr_err("%s: Error! ping pong mismiatch!!", __func__);
}
//LGE_CHANGE_E, Workaround Sof Freeze patch
