...
#define CCI_TIMEOUT msecs_to_jiffies(500) //LGE_UPDATE 100->500
	...
	enum cci_i2c_sync sync_en)
{
	...
	uint32_t reg_offset;
	uint32_t val = 0;
	uint32_t max_queue_size, queue_size = 0; //LGE_UPDATE for sequential writing 20160219 yt.jeon@lge.com
	...
	max_queue_size = cci_dev->cci_i2c_queue_info[master][queue].
			max_queue_size;
//LGE_UPDATE_S for sequential writing 20160219 yt.jeon@lge.com
	if (c_ctrl->cmd == MSM_CCI_I2C_WRITE_SEQ)
		queue_size = max_queue_size;
	else
		queue_size = max_queue_size/2;
//LGE_UPDATE for sequential writing 20160219 yt.jeon@lge.com
