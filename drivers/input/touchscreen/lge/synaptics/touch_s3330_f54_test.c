...
unsigned char TRX_Short[7] = {0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
int ImageTest(struct device *dev, int mode, char *buf)
{
	...
	data = eRT_RawImageRT3;	/* Raw Capacitance mode */
...
int DeltaTest(struct device *dev, int mode, char *buf)
{
	...
	return ReadReport(dev, report_type, mode, buf);
}
...
*/
int SensorSpeed(struct device *dev, int mode, char *buf)
{
	unsigned char data;
	u16 report_type;
...
void AbsADCRange(struct device *dev, int mode, char *buf)
{
	unsigned char data;
	u16 report_type;
	...
	if (TestPreparation(dev)) {
		...
		ReadReport(dev, report_type, mode, buf);
	}
}

/* report type 40 */
int AbsDelta(struct device *dev, int mode, char *buf)
{
	unsigned char data;
	u16 report_type;
...
*/
void TRexOpenTest(struct device *dev, int mode, char *buf)
{
	unsigned char data;
	u16 report_type;
...
*/
int TRexGroundTest(struct device *dev, int mode, char *buf)
{
	unsigned char data;
	u16 report_type;
	int ret;
...
void MaxMinTest(struct device *dev, int mode, char *buf)
{
	unsigned char data;
	u16 report_type;
...
retry:
	...
	case eRT_RawImageRT3:
		...
		break;
	case eRT_ADCRange:
		ret = ADCRange(dev, mode, buf);
		break;
	case eRT_SensorSpeed:
		ret = SensorSpeed(dev, mode, buf);
		break;
	case eRT_TRexOpen:
		TRexOpenTest(dev, mode, buf);
		break;
	case eRT_TRexGround:
		ret = TRexGroundTest(dev, mode, buf);
		break;
	case eRT_TRexShort:
	...
	case eRT_HighResistance:
		...
		break;
	case eRT_FullRawCapacitanceMaxMin:
		MaxMinTest(dev, mode, buf);
		break;
	case eRT_AbsADCRange:
		AbsADCRange(dev, mode, buf);
		break;
	case eRT_AbsDelta:
		ret = AbsDelta(dev, mode, buf);
		break;
	case eRT_AbsRaw:
