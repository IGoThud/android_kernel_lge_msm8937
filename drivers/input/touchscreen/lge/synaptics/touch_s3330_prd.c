...
static void production_info_log(struct device *dev)
{
	...
	ret += snprintf(buffer + ret, LOG_BUF_SIZE - ret,
				"Program_Name : 0x");
	for (i = 0; i < 5; i++) {
		ret += snprintf(buffer + ret, LOG_BUF_SIZE - ret, "%02X",
				data[i + 1]);
	}
...
static int synaptics_abs_rawdata_test(struct device *dev)
{
	...
	ret = F54Test(dev, eRT_AbsRaw, 2, NULL);
...
static int synaptics_sd(struct device *dev, char *buf)
{
	...
	if (lower_limit < 0 || upper_limit < 0) {
		...
		abs_rawdata_ret = synaptics_abs_rawdata_test(dev);
		touch_msleep(20);
	...
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "Raw Data : %s",
				(rawdata_ret == 1 && abs_rawdata_ret == 1)
				? "Pass\n" : "Fail ");

	if (rawdata_ret != 1 || abs_rawdata_ret != 1) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "(%s/%s)\n",
					(rawdata_ret != 1 ? "0" : "1"),
					(abs_rawdata_ret != 1 ? "0" : "1"));
	}
...
static ssize_t show_delta(struct device *dev, char *buf)
{
	...
	if (synaptics_is_product(d, "PLG591", 6))
		...
		TOUCH_E("Show_delta Test Error!!(product_id error)\n");

	if (ret == 0) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"Test Result : delta fail.\n");
	}
...
static ssize_t show_rawdata(struct device *dev, char *buf)
{
	...
	if (synaptics_is_product(d, "PLG591", 6))
		...
		TOUCH_E("Show_rawdata Test Error!!(product_id error)\n");

	if (ret == 0) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
					"Test Result : rawdata fail.\n");
	}
...
static ssize_t show_chstatus(struct device *dev, char *buf)
{
	...
	if (synaptics_is_product(d, "PLG591", 6)) {
		...
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "High Resistance result : %s",
					(high_resistance_ret == 1) ? "Pass\n" : "Fail\n");
	} else {
		ret = snprintf(buf + ret, PAGE_SIZE - ret, "product_id error!!!\n");
	}
...
static ssize_t show_abs_rawdata(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct synaptics_data *d = to_synaptics_data(dev);
	...
	if (d->power_state == POWER_SLEEP) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"LCD off!!!. Can not abs_rawdata_test.\n");
		return ret;
	}
	...
	if (synaptics_is_product(d, "PLG591", 6)) {
		...
		abs_rawdata_ret = F54Test(dev, eRT_AbsRaw, 2, NULL);

		mode = 0x00;
		synaptics_write(dev, ABS_DOZE_CTRL_REG, &mode, sizeof(mode));

		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Abs Sensing Raw Capacitance Test : %s",
				(abs_rawdata_ret == 1) ? "Pass\n" : "Fail\n");
...
static ssize_t show_lpwg_abs_rawdata(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct synaptics_data *d = to_synaptics_data(dev);
	...
	if (d->power_state == POWER_WAKE) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"LCD on!!!. Can not abs_lpwg_rawdata_test.\n");
		return ret;
	}
	...
	if (synaptics_is_product(d, "PLG591", 6)) {
		touch_msleep(100);
		lpwg_abs_rawdata_ret = F54Test(dev, eRT_AbsRaw, 3, NULL);

		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"LPWG RawData : %s",
				(lpwg_abs_rawdata_ret == 1) ? "Pass\n" : "Fail\n");
	} else {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "product_id error!!!\n");
	}
...
static ssize_t show_abs_chstatus(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct synaptics_data *d = to_synaptics_data(dev);

	int ret = 0;
	int abs_raw_short_ret = 0;
	int abs_raw_open_ret = 0;

	TOUCH_I("%s\n", __func__);

	if (d->power_state == POWER_SLEEP) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"LCD off!!!. Can not abs_chstatus_test.\n");
		return ret;
	}
	...
	if (synaptics_is_product(d, "PLG591", 6)) {
		abs_raw_short_ret = F54Test(dev, eRT_AbsRaw, 0, NULL);
		abs_raw_open_ret = F54Test(dev, eRT_AbsRaw, 1, NULL);

		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Absolute Sensing Short Test result : %s",
				(abs_raw_short_ret == 1) ? "Pass\n" : "Fail\n");

		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"Absolute Sensing Open Test result : %s",
				(abs_raw_open_ret == 1) ? "Pass\n" : "Fail\n");
	} else {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "product_id error!!!\n");
	}
...
static ssize_t show_sensor_speed(struct device *dev, char *buf)
{
	...
	int lower_sensor_limit = 0;
	int upper_sensor_limit = 0;

	TOUCH_I("%s\n", __func__);

	if (d->power_state == POWER_SLEEP) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"LCD off!!!. Can not sensor_speed_test.\n");
		return ret;
	}
	...
	lower_sensor_limit = synaptics_get_limit(dev, "SensorSpeedLowerImageLimit",
				TxChannelCount, RxChannelCount, SensorSpeedLowerImage);
	upper_sensor_limit = synaptics_get_limit(dev, "SensorSpeedUpperImageLimit",
				TxChannelCount, RxChannelCount, SensorSpeedUpperImage);

	if (lower_sensor_limit < 0 || upper_sensor_limit < 0) {
		TOUCH_E("[Fail] lower_limit = %d, upper_limit = %d\n",
				lower_sensor_limit, upper_sensor_limit);
		TOUCH_E("[Fail] Can not check the limit of Sensor Speed image\n");
		ret = snprintf(buf + ret, PAGE_SIZE - ret,
				"Can not check the limit of Sensor Speed image\n");
	} else {
		TOUCH_I("[Success] Can check the limit of Sensor Speed image\n");

		ret += snprintf(buf + ret, PAGE_SIZE - ret, "========RESULT=======\n");

		if (synaptics_is_product(d, "PLG591", 6)) {
			sensor_speed_ret = F54Test(dev, eRT_SensorSpeed, 0, NULL);

			ret += snprintf(buf + ret, PAGE_SIZE - ret,
						"Sensor Speed Test result : %s",
						(sensor_speed_ret == 1) ? "Pass\n" : "Fail\n");
		} else {
			ret += snprintf(buf + ret, PAGE_SIZE - ret,
						"product_id error!!!\n");
		}
...
static ssize_t show_adc_range(struct device *dev, char *buf)
{
	...
	int ret = 0;
	int adc_range_ret = 0;
	int lower_adc_limit = 0;
	int upper_adc_limit = 0;

	TOUCH_I("%s\n", __func__);

	if (d->power_state == POWER_SLEEP) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
				"LCD off!!!. Can not adc_range_test.\n");
		return ret;
	}
	...
	lower_adc_limit = synaptics_get_limit(dev, "ADCLowerImageLimit",
				TxChannelCount, RxChannelCount, ADCLowerImage);
	upper_adc_limit = synaptics_get_limit(dev, "ADCUpperImageLimit",
				TxChannelCount, RxChannelCount, ADCUpperImage);

	if (lower_adc_limit < 0 || upper_adc_limit < 0) {
		TOUCH_E("[Fail] lower_limit = %d, upper_limit = %d\n",
				lower_adc_limit, upper_adc_limit);
		TOUCH_E("[Fail] Can not check the limit of ADC image\n");
		ret = snprintf(buf + ret, PAGE_SIZE - ret,
				"Can not check the limit of ADC image\n");
	} else {
		TOUCH_I("[Success] Can check the limit of ADC image\n");

		ret += snprintf(buf + ret, PAGE_SIZE - ret, "========RESULT=======\n");

		if (synaptics_is_product(d, "PLG591", 6)) {
			adc_range_ret = F54Test(dev, eRT_ADCRange, 0, NULL);

			ret += snprintf(buf + ret, PAGE_SIZE - ret,
						"ADC Range Test result : %s",
						(adc_range_ret == 1) ? "Pass\n" : "Fail\n");
		} else {
			ret += snprintf(buf + ret, PAGE_SIZE - ret,
						"product_id error!!!\n");
		}
...
static ssize_t show_trx_gnd(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct synaptics_data *d = to_synaptics_data(dev);
	...
	if (synaptics_is_product(d, "PLG591", 6)) {
		trx_gnd_ret = F54Test(dev, eRT_TRexGround, 0, NULL);

		ret += snprintf(buf + ret, PAGE_SIZE - ret, "TRX Gnd Test result : %s",
					(trx_gnd_ret == 1) ? "Pass\n" : "Fail\n");
	} else {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "product_id error!!!\n");
	}
...
static TOUCH_ATTR(lpwg_abs_rawdata, show_lpwg_abs_rawdata, NULL);
static TOUCH_ATTR(abs_rawdata, show_abs_rawdata, NULL);
static TOUCH_ATTR(abs_chstatus, show_abs_chstatus, NULL);
static TOUCH_ATTR(sensor_speed, show_sensor_speed, NULL);
static TOUCH_ATTR(adc_range, show_adc_range, NULL);
static TOUCH_ATTR(trx_gnd, show_trx_gnd, NULL);
...
static struct attribute *prd_attribute_list[] = {
	...
	&touch_attr_lpwg_abs_rawdata.attr,
	&touch_attr_abs_rawdata.attr,
	&touch_attr_abs_chstatus.attr,
	&touch_attr_sensor_speed.attr,
	&touch_attr_adc_range.attr,
	&touch_attr_trx_gnd.attr,
