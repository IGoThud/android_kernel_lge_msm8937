...
static int synaptics_tci_password(struct device *dev)
{
	...
	info->tap_count &= 0x0f;
	info->intr_delay = ts->tci.double_tap_check ? 68 : 0; // 55 (Interval to Doze : 600)
	info->tap_distance = 7;
...
int synaptics_init_reg(struct device *dev)
{
	...
	ret = synaptics_set_lpwg_pmic_delay(dev); // Force update is done in next LPWG enterance
...
static void synaptics_init_tci_info(struct device *dev)
{
	...
	ts->tci.info[1].intr_delay = 68; // 55 (Interval to Doze : 600)
}
...
int synaptics_soft_reset(struct device *dev)
{
	...
	synaptics_irq_clear(dev); // Only configured in done
...
static void synaptics_esd_recovery_work_func(struct work_struct *esd_recovery_work)
{
	...
	if (d->state != TC_STATE_ACTIVE) {
		TOUCH_E("esd_recovery_work is called in NOT TC_STATE_ACTIVE\n");
		goto RET;
	}
...
int synaptics_set_tuning(struct device *dev)
{
	...
	// ALGM
	data[0] = 0x01; // ALGM En
	data[1] = 0x07; // Min Comp Factor
	ret = synaptics_write(dev, 0x23, data, 2);

	// 2D Ctrl
	data[0] = 30; // Noise Floor
	data[1] = 40; // Min Peak Amp
	ret = synaptics_write(dev, 0x17, data, 2);
