...
int td4300_prd_get_open_test_data(struct device *dev)
{
	...
	ret = synaptics_set_page(dev, ANALOG_PAGE);
	ret |= synaptics_read(dev, 0x3B, ref_transcap, sizeof(ref_transcap)); // F54_ANALOG_CTRL91(Analog Control 2)
	ret |= synaptics_read(dev, 0x3F, cbc_transcap, sizeof(cbc_transcap)); // F54_ANALOG_CTRL96(CBC Transcap)
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		goto FAIL;
	}
	...
	ret = synaptics_write(dev, 0x3B, buf, sizeof(ref_transcap)); // F54_ANALOG_CTRL91(Analog Control 2)
	ret |= synaptics_set_page(dev, DEFAULT_PAGE);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		goto FAIL;
	...
	ret = synaptics_set_page(dev, ANALOG_PAGE);
	ret |= synaptics_write(dev, 0x3F, buf, sizeof(cbc_transcap)); // F54_ANALOG_CTRL96(CBC Transcap)
	ret |= synaptics_set_page(dev, DEFAULT_PAGE);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		goto FAIL;
	...
	// Restore Transcap
	ret = synaptics_set_page(dev, ANALOG_PAGE);
	ret |= synaptics_write(dev, 0x3B, ref_transcap, sizeof(ref_transcap)); // F54_ANALOG_CTRL91(Analog Control 2)
	ret |= synaptics_write(dev, 0x3F, cbc_transcap, sizeof(cbc_transcap)); // F54_ANALOG_CTRL96(CBC Transcap)
	ret |= synaptics_set_page(dev, DEFAULT_PAGE);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		goto FAIL;
