...
#if defined(CONFIG_LGE_TOUCH_SYNAPTICS_TD4300)
#include <synaptics/touch_td4300.h>
#endif
...
#if defined(CONFIG_LGE_TOUCH_FOCALTECH_FT8707) || defined(CONFIG_LGE_TOUCH_SYNAPTICS_TD4300)
	// lock is done in ts->driver->lpwg. since fb_lock is added
	if (ts->driver->lpwg) {
